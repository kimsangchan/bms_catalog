...
/* 레일 3단째 — 모델. sub(2단)보다 한 단 더 들여 계층이 눈에 보이게 한다 */
aside button.msubb{padding-left:38px;font-size:11.4px}
aside button.msubb[aria-current="true"]{color:var(--accent);font-weight:650;
 border-left-color:var(--accent);background:var(--sel)}
...
function buildNav(){
  ...
  if(axis==='eq'){
    D.domOrder.forEach(function(dm){
      ...
      list.forEach(function(e){
        ...
        // 모델은 **고른 계열 아래에만** 편다. 냉동기는 70건이라 늘 펼치면 레일이
        // 스크롤 전용이 되고, 셀렉트로만 닿던 모델이 트리에서 안 보이는 문제가 남는다.
        h += '<div class="msub" data-for="'+e.id+'"></div>';
      });
    });
  } else {
    var vx = vendorIndex();
  ...
  // 레일의 모델 단추는 렌더마다 다시 만들어지므로 상위에서 위임으로 받는다.
  // 초기화 항목은 본문 셀렉트(.msel)와 똑같이 맞춘다 — 안 그러면 판·표 선택이 남는다.
  if(!nav.dataset.msubWired){
    nav.dataset.msubWired = '1';
    nav.addEventListener('click', function(ev){
      var b = ev.target.closest && ev.target.closest('.msubb');
      if(!b) return;
      mi=+b.dataset.mi; vsel=0; ssel=0; mview='unit'; ksel='rating'; pageOf={}; render();
    });
  }
}
...
function markNav(){ nav.querySelectorAll('button:not(.axb):not(.msubb)').forEach(function(b){
  b.setAttribute('aria-current', b.dataset.id===cur && (b.dataset.sub||'')===subF
    && (b.dataset.vendor||'')===vnF ? 'true':'false'); });
  renderNavModels(); }

// 레일 3단째 — 고른 계열의 모델. 본문 셀렉트/단추와 같은 것을 가리키고 서로 따라간다.
function renderNavModels(){
  nav.querySelectorAll('.msub').forEach(function(box){
    if(box.dataset.for !== String(cur)){ box.innerHTML=''; return; }
    var models = visibleModels(cur) || [];
    if(models.length<2){ box.innerHTML=''; return; }   // 하나뿐이면 단계를 만들지 않는다
    var ci = Math.min(mi, models.length-1);
    box.innerHTML = models.map(function(x,i){
      var n = (x.points||[]).length;
      return '<button class="sub msubb" data-mi="'+i+'" aria-current="'+(i===ci)+'"'
        + ' title="'+esc(x.vendor+' · '+(x.model||''))+'">└ '+esc(x.selectorLabel||x.model)
        + '<i>'+(n?n+'점':'')+'</i></button>'; }).join('');
  });
}
