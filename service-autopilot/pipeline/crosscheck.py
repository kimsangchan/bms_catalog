...
def _key(s):
    ...
    ⚠ 하이픈은 ASCII 만이 아니다. 조판용 U+2010(줄바꿈 하이픈)·U+2011(비분리
    하이픈)을 쓰는 문서가 있다(JCI YKN2Open). 줄 읽기 경로는 거기서 이름을
    'Suction Temper‐' 로 자르는데, _same 의 포함 관계 완화가 꼬리의 '‐' 때문에
    걸리지 않아 **멀쩡한 13건이 불일치로 잡혔다**(일치율 71%). 비교용 열쇠일 뿐
    저장값은 건드리지 않는다.
    """
