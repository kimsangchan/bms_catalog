...
5.2 **사전이 먼저다.** 새 필드는 `data/point-schema.json`, 새 정격 속성은
   `data/unit-schema.json`, **새 BMS 화면 개념은 `data/point-concepts.json`** 에 먼저
   등재한다. 템플릿 행은 이름 문자열이 아니라 개념 id 를 가리킨다 — 이름으로 두었더니
   같은 개념이 계열마다 다르게 적혀 10건이 갈렸다(운전/정지 지령 ↔ 기동/정지 지령,
   필터 차압 ↔ 필터 청소 신호). 계열 19개면 행이 수백이라 손으로는 못 막는다.
