...
- **[보류]** YS 판 2개를 YN 모델에서 YS 모델로 옮기는 건 **접었다**(2026-08-20 시도).
  구현해 돌려 보니 둘이 깨진다 — ⑴ 판 id 를 모델별로 다시 매겨 `ys-yn-optiview-revk-03-p1`
  이 두 모델에 생긴다(validate 도 못 잡는다) ⑵ 문서의 판이 두 모델로 갈리면 `--refresh`
  가 문서 전체 맥락을 못 봐 `appliesTo` 좁힘이 다음 회차에 풀린다. id 는 판 이름 슬러그로
  고칠 수 있지만 ⑵ 는 "판을 (모델, 문서)로 묶어 다시 짜는" 구조 자체의 문제다.
  지금도 `appliesTo` 와 판 이름이 제품을 밝히고 있어 찾는 데 지장은 없다.
